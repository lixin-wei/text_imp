<?php
    ...
    /* Get review START */
    $date_start = (new DateTime())->modify("-29 days")->format("Y-m-d");
    ...
    if ($rate != 0) $where.=" AND review.rate = $rate";
...
SQL;
    ...
    //app列表
    $sql = "SELECT app_id, app_name, store FROM app";
    $app_list = $db->query($sql)->fetch_all(MYSQLI_ASSOC);
    //版本列表
    $sql = "SELECT DISTINCT version FROM review";
    if($app_id!=0)$sql .= " WHERE app_id=$app_id";
    $version_list = $db->query($sql)->fetch_all(MYSQLI_NUM);
    //省份列表
    $sql = "SELECT DISTINCT location FROM review";
    if($app_id!=0)$sql .= " WHERE app_id = $app_id";
